#include <stdint.h>
#include <setjmp.h>
#include <stdarg.h>
#include <stddef.h>
#include <cmocka.h>
#include <stdio.h>
#include <limits.h>
#include <math.h>

#include <stdlib.h>
#include <unistd.h>
#include <sys/types.h>
#include <sys/stat.h>
#include <fcntl.h>
#include <sys/wait.h>

#include "instruction.h"

#define MAX_ITERATION 1000
#define MEMORY_SIZE 1024 * 1024

//
static core_t *core_init()
{
    core_t *core = (core_t *)malloc(sizeof(core_t));
    if (core == NULL)
    {
        printf("failed to allocate core of size %zu bytes", sizeof(core_t));
        exit(1);
    }

    for (int i = 0; i < NUMBER_SCALAR_REGISTER; ++i)
    {
        core->U[i] = 0;
        core->S[i] = 0;
        core->F[i] = 0.0;
    }

    for (int i = 0; i < 8; i++)
    {
        core->CF[i] = false;
    }

    core->IP = 0;

    return core;
}

//
static u32 create_instruction(u8 opcode, u16 offset, u8 register_1, u8 register_2, u8 register_3)
{
    return htobe32((opcode & 0xFF) << (SIZE_INSTRUCTION - 8) | (offset & 0x1FF) << (SIZE_INSTRUCTION - 17) | (register_1 & 0x1F) << (SIZE_INSTRUCTION - 22) | (register_2 & 0x1F) << (SIZE_INSTRUCTION - 27) | (register_3 & 0x1F));
}

// u1 = memory(u2 + u3 + offset)
static void test_loadu(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    u8 *memory = (u8 *)malloc(MEMORY_SIZE);
    if (memory == NULL)
    {
        printf("failed to allocate memory of size %u bytes", MEMORY_SIZE);
        free(file_buffer);
        free(core);
        exit(1);
    }

    core->memory = memory;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        // intialisation des registres
        u8 r1 = (u8)(rand() % 10);        // entre 0 et 9
        u8 r2 = (u8)(10 + (rand() % 10)); // entre 10 et 19
        u8 r3 = (u8)(20 + (rand() % 12)); // entre 20 et 31

        core->U[r2] = (u64)(rand() % (MEMORY_SIZE / 3));
        core->U[r3] = (u64)(rand() % (MEMORY_SIZE / 3));
        u8 offset = (u8)rand();

        u32 *ptr = (u32 *)(core->file_buffer + core->IP);
        *ptr = create_instruction(0, offset, r1, r2, r3);

        u64 value = (u64)rand();
        u64 indice = core->U[r2] + core->U[r3] + offset;

        *(u64 *)(core->memory + indice) = value;

        loadu(core);
        core->IP = 0;

        assert_int_equal(core->U[r1], value);
    }

    core_drop(core);
}

// s1 = memory(u2 + u3 + offset)
static void test_loads(void **state)
{

    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    u8 *memory = (u8 *)malloc(MEMORY_SIZE);
    if (memory == NULL)
    {
        printf("failed to allocate memory of size %u bytes", MEMORY_SIZE);
        free(file_buffer);
        free(core);
        exit(1);
    }

    core->memory = memory;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        // intialisation des registres
        u8 r1 = (u8)(rand() % 32);        // entre 0 et 32
        u8 r2 = (u8)((rand() % 15));      // entre 0 et 14
        u8 r3 = (u8)(15 + (rand() % 17)); // entre 15 et 31

        core->U[r2] = (u64)(rand() % (MEMORY_SIZE / 3));
        core->U[r3] = (u64)(rand() % (MEMORY_SIZE / 3));
        u8 offset = (u8)rand();

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, offset, r1, r2, r3);

        i64 value = (i64)rand();
        u64 indice = core->U[r2] + core->U[r3] + offset;

        *(i64 *)(core->memory + indice) = value;

        loads(core);
        core->IP = 0;

        assert_int_equal(core->S[r1], value);
    }

    core_drop(core);
}

// f1 = memory(u2 + u3 + offset)
static void test_loadf(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    u8 *memory = (u8 *)malloc(MEMORY_SIZE);
    if (memory == NULL)
    {
        printf("failed to allocate memory of size %u bytes", MEMORY_SIZE);
        free(file_buffer);
        free(core);
        exit(1);
    }

    core->memory = memory;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        // intialisation des registres
        u8 r1 = (u8)(rand() % 32);        // entre 0 et 32
        u8 r2 = (u8)((rand() % 15));      // entre 0 et 14
        u8 r3 = (u8)(15 + (rand() % 17)); // entre 15 et 31

        core->U[r2] = (u64)(rand() % (MEMORY_SIZE / 3));
        core->U[r3] = (u64)(rand() % (MEMORY_SIZE / 3));
        u8 offset = (u8)rand();

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, offset, r1, r2, r3);

        f64 value = (f64)(rand() / 2.2814);
        u64 indice = core->U[r2] + core->U[r3] + offset;

        *(f64 *)(core->memory + indice) = value;

        loadf(core);
        core->IP = 0;

        assert_float_equal(core->F[r1], value, 0.0001);
    }

    core_drop(core);
}

// v1 = memory(u2 + u3 + offset)
static void test_loadv(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    u8 *memory = (u8 *)malloc(MEMORY_SIZE);
    if (memory == NULL)
    {
        printf("failed to allocate memory of size %u bytes", MEMORY_SIZE);
        free(file_buffer);
        free(core);
        exit(1);
    }

    core->memory = memory;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        // intialisation des registres
        u8 r1 = (u8)(rand() % 32);        // entre 0 et 32
        u8 r2 = (u8)((rand() % 15));      // entre 0 et 14
        u8 r3 = (u8)(15 + (rand() % 17)); // entre 15 et 31

        core->U[r2] = (u64)(rand() % (MEMORY_SIZE / 4));
        core->U[r3] = (u64)(rand() % (MEMORY_SIZE / 4));
        u8 offset = (u8)rand();

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, offset, r1, r2, r3);

        u64 indice = core->U[r2] + core->U[r3] + offset;

        for (u64 i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            *(u64 *)(core->memory + indice + (u64)(i * sizeof(u64))) = (u64)random();
        }

        loadv(core);
        core->IP = 0;

        for (u64 i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            assert_int_equal(core->V[r1][i], *(u64 *)(core->memory + indice + (i * sizeof(u64))));
        }
    }

    core_drop(core);
}

// t1 = memory(u2 + u3 + offset)
static void test_loadt(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    u8 *memory = (u8 *)malloc(MEMORY_SIZE);
    if (memory == NULL)
    {
        printf("failed to allocate memory of size %u bytes", MEMORY_SIZE);
        free(file_buffer);
        free(core);
        exit(1);
    }

    core->memory = memory;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        // intialisation des registres
        u8 r1 = (u8)(rand() % 32);        // entre 0 et 32
        u8 r2 = (u8)((rand() % 15));      // entre 0 et 14
        u8 r3 = (u8)(15 + (rand() % 17)); // entre 15 et 31

        core->U[r2] = (u64)(rand() % (MEMORY_SIZE / 4));
        core->U[r3] = (u64)(rand() % (MEMORY_SIZE / 4));
        u8 offset = (u8)rand();

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, offset, r1, r2, r3);

        u64 indice = core->U[r2] + core->U[r3] + offset;

        for (u64 i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            *(i64 *)(core->memory + indice + (i * sizeof(i64))) = (i64)random();
        }

        loadt(core);
        core->IP = 0;

        for (u64 i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            assert_int_equal(core->T[r1][i], *(i64 *)(core->memory + indice + (i * sizeof(i64))));
        }
    }

    core_drop(core);
}

// g1 = memory(u2 + u3 + offset)
static void test_loadg(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    u8 *memory = (u8 *)malloc(MEMORY_SIZE);
    if (memory == NULL)
    {
        printf("failed to allocate memory of size %u bytes", MEMORY_SIZE);
        free(file_buffer);
        free(core);
        exit(1);
    }

    core->memory = memory;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        // intialisation des registres
        u8 r1 = (u8)(rand() % 32);        // entre 0 et 32
        u8 r2 = (u8)((rand() % 15));      // entre 0 et 14
        u8 r3 = (u8)(15 + (rand() % 17)); // entre 15 et 31

        core->U[r2] = (u64)(rand() % (MEMORY_SIZE / 4));
        core->U[r3] = (u64)(rand() % (MEMORY_SIZE / 4));
        u8 offset = (u8)rand();

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, offset, r1, r2, r3);

        u64 indice = core->U[r2] + core->U[r3] + offset;

        for (u64 i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            *(f64 *)(core->memory + indice + (i * sizeof(f64))) = (f64)random();
        }

        loadg(core);
        core->IP = 0;

        for (u64 i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            assert_float_equal(core->G[r1][i], *(f64 *)(core->memory + indice + (i * sizeof(f64))), 0.0001);
        }
    }

    core_drop(core);
}

// memory(u1 + u2 + offset) = u3
static void test_storeu(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    u8 *memory = (u8 *)malloc(MEMORY_SIZE);
    if (memory == NULL)
    {
        printf("failed to allocate memory of size %u bytes", MEMORY_SIZE);
        free(core);
        free(memory);
        exit(1);
    }

    core->memory = memory;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        // intialisation des registres
        u8 r1 = (u8)(rand() % 10);        // entre 0 et 9
        u8 r2 = (u8)(10 + (rand() % 10)); // entre 10 et 19
        u8 r3 = (u8)(20 + (rand() % 12)); // entre 20 et 31
        // indice
        core->U[r1] = (u64)(rand() % (MEMORY_SIZE / 3));
        core->U[r2] = (u64)(rand() % (MEMORY_SIZE / 3));
        u8 offset = (u8)rand();

        // value
        core->U[r3] = (u64)(rand() % (MEMORY_SIZE / 3));
        u32 *ptr = (u32 *)(core->file_buffer + core->IP);
        *ptr = create_instruction(0, offset, r1, r2, r3);

        u64 indice = core->U[r1] + core->U[r2] + offset;

        storeu(core);
        core->IP = 0;

        assert_int_equal(core->U[r3], *(u64 *)(core->memory + indice));
    }

    core_drop(core);
}

// memory(u1 + u2 + offset) = s3
static void test_stores(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    u8 *memory = (u8 *)malloc(MEMORY_SIZE);
    if (memory == NULL)
    {
        printf("failed to allocate memory of size %u bytes", MEMORY_SIZE);
        free(core);
        free(memory);
        exit(1);
    }

    core->memory = memory;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        // intialisation des registres
        u8 r1 = (u8)(rand() % 15);        // entre 0 et 14
        u8 r2 = (u8)(15 + (rand() % 17)); // entre 15 et 31
        u8 r3 = (u8)(rand() % 32);        // entre 0 et 31

        // indice
        core->U[r1] = (u64)(rand() % (MEMORY_SIZE / 3));
        core->U[r2] = (u64)(rand() % (MEMORY_SIZE / 3));
        u8 offset = (u8)rand();

        // value (taille de la mémoire doit au moins être 8 octets)
        core->S[r3] = (i64)rand();

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, offset, r1, r2, r3);

        u64 indice = core->U[r1] + core->U[r2] + offset;

        stores(core);
        core->IP = 0;

        assert_int_equal(core->S[r3], *(i64 *)(core->memory + indice));
    }

    core_drop(core);
}

// memory(u1 + u2 + offset) = f3
static void test_storef(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    u8 *memory = (u8 *)malloc(MEMORY_SIZE);
    if (memory == NULL)
    {
        printf("failed to allocate memory of size %u bytes", MEMORY_SIZE);
        free(core);
        free(memory);
        exit(1);
    }

    core->memory = memory;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        // intialisation des registres
        u8 r1 = (u8)(rand() % 15);        // entre 0 et 14
        u8 r2 = (u8)(15 + (rand() % 17)); // entre 15 et 31
        u8 r3 = (u8)(rand() % 32);        // entre 0 et 31

        // indice
        core->U[r1] = (u64)(rand() % (MEMORY_SIZE / 3));
        core->U[r2] = (u64)(rand() % (MEMORY_SIZE / 3));
        u8 offset = (u8)rand();

        // value (taille de la mémoire doit au moins être 8 octets)
        core->F[r3] = (f64)(rand() / 2.1541);

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, offset, r1, r2, r3);

        u64 indice = core->U[r1] + core->U[r2] + offset;

        storef(core);
        core->IP = 0;

        assert_float_equal(core->F[r3], *(f64 *)(core->memory + indice), 0.0001);
    }

    core_drop(core);
}

//  memory(u1 + u2 + offset) = v3
static void test_storev(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    u8 *memory = (u8 *)malloc(MEMORY_SIZE);
    if (memory == NULL)
    {
        printf("failed to allocate memory of size %u bytes", MEMORY_SIZE);
        free(file_buffer);
        free(core);
        exit(1);
    }

    core->memory = memory;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        // intialisation des registres
        u8 r1 = (u8)(rand() % 10);        // entre 0 et 9
        u8 r2 = (u8)(10 + (rand() % 10)); // entre 10 et 19
        u8 r3 = (u8)(20 + (rand() % 12)); // entre 20 et 31
        // indice
        core->U[r1] = (u64)(rand() % (MEMORY_SIZE / 3));
        core->U[r2] = (u64)(rand() % (MEMORY_SIZE / 3));
        u8 offset = (u8)rand();

        u32 *ptr = (u32 *)(core->file_buffer + core->IP);
        *ptr = create_instruction(0, offset, r1, r2, r3);

        u64 indice = core->U[r1] + core->U[r2] + offset;

        for (u64 i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            core->V[r3][i] = (u64)(rand() % (MEMORY_SIZE / 3));
        }

        storev(core);
        core->IP = 0;

        for (u64 i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            assert_int_equal(core->V[r3][i], *(u64 *)(core->memory + indice + (i * sizeof(u64))));
        }
    }

    core_drop(core);
}

// memory(u1 + u2 + offset) = t3
static void test_storet(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    u8 *memory = (u8 *)malloc(MEMORY_SIZE);
    if (memory == NULL)
    {
        printf("failed to allocate memory of size %u bytes", MEMORY_SIZE);
        free(file_buffer);
        free(core);
        exit(1);
    }

    core->memory = memory;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        // intialisation des registres
        u8 r1 = (u8)(rand() % 10);        // entre 0 et 9
        u8 r2 = (u8)(10 + (rand() % 10)); // entre 10 et 19
        u8 r3 = (u8)(20 + (rand() % 12)); // entre 20 et 31
        // indice
        core->U[r1] = (u64)(rand() % (MEMORY_SIZE / 3));
        core->U[r2] = (u64)(rand() % (MEMORY_SIZE / 3));
        u8 offset = (u8)rand();

        u32 *ptr = (u32 *)(core->file_buffer + core->IP);
        *ptr = create_instruction(0, offset, r1, r2, r3);

        u64 indice = core->U[r1] + core->U[r2] + offset;

        for (u64 i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            core->T[r3][i] = (i64)(rand() % (MEMORY_SIZE / 3));
        }

        storet(core);
        core->IP = 0;

        for (u64 i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            assert_int_equal(core->T[r3][i], *(i64 *)(core->memory + indice + (i * sizeof(i64))));
        }
    }

    core_drop(core);
}

// memory(u1 + u2 + offset) = g3
static void test_storeg(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    u8 *memory = (u8 *)malloc(MEMORY_SIZE);
    if (memory == NULL)
    {
        printf("failed to allocate memory of size %u bytes", MEMORY_SIZE);
        free(file_buffer);
        free(core);
        exit(1);
    }

    core->memory = memory;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        // intialisation des registres
        u8 r1 = (u8)(rand() % 10);        // entre 0 et 9
        u8 r2 = (u8)(10 + (rand() % 10)); // entre 10 et 19
        u8 r3 = (u8)(20 + (rand() % 12)); // entre 20 et 31
        // indice
        core->U[r1] = (u64)(rand() % (MEMORY_SIZE / 3));
        core->U[r2] = (u64)(rand() % (MEMORY_SIZE / 3));
        u8 offset = (u8)rand();

        u32 *ptr = (u32 *)(core->file_buffer + core->IP);
        *ptr = create_instruction(0, offset, r1, r2, r3);

        u64 indice = core->U[r1] + core->U[r2] + offset;

        for (u64 i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            core->G[r3][i] = (f64)(rand() % (MEMORY_SIZE / 3));
        }

        storeg(core);
        core->IP = 0;

        for (u64 i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            assert_int_equal(core->G[r3][i], *(f64 *)(core->memory + indice + (i * sizeof(f64))));
        }
    }

    core_drop(core);
}

// u1 = u2
static void test_movu(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    // U[r1] = U[r2]
    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 15);      // Entre 0 et 14
        u8 r2 = (u8)(15 + random() % 17); // Entre 15 et 31

        u64 v1 = (u64)((rand() % (RAND_MAX / 2)) + (RAND_MAX / 2));
        u64 v2 = (u64)(rand() % (RAND_MAX / 2));

        core->U[r1] = v1;
        core->U[r2] = v2;

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, 0);

        assert_int_not_equal(v1, v2);

        movu(core);
        core->IP = 0;

        assert_int_equal(core->U[r1], core->U[r2]);
        assert_int_equal(v2, core->U[r2]);
    }

    // cas extreme des u64 ?

    // cas ou c'est pas des u64 ?
    free(file_buffer);
    free(core);
}

// s1 = s2
static void test_movs(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;


    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 15);      // Entre 0 et 14
        u8 r2 = (u8)(15 + random() % 17); // Entre 15 et 31

        i64 v1 = (i64)((rand() % (RAND_MAX / 2)) + (RAND_MAX / 2));
        i64 v2 = (i64)(rand() % (RAND_MAX / 2));

        core->S[r1] = v1;
        core->S[r2] = v2;

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, 0);

        assert_int_not_equal(v1, v2);

        movs(core);
        core->IP = 0;

        assert_int_equal(core->S[r1], core->S[r2]);
        assert_int_equal(v2, core->S[r2]);
    }

    // cas extreme des u64 ?

    // cas ou c'est pas des u64 ?
    free(file_buffer);
    free(core);
}

// f1 = f2
static void test_movf(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    for (int i = 0; i < 1; ++i)
    {
        u8 r1 = (u8)(random() % 15);      // Entre 0 et 14
        u8 r2 = (u8)(15 + random() % 17); // Entre 15 et 31

        f64 v1 = (f64)((rand() % (RAND_MAX / 2)) + (RAND_MAX / 2)) / 2.2156;
        f64 v2 = (f64)(rand() % (RAND_MAX / 2));

        core->F[r1] = v1;
        core->F[r2] = v2;

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, 0);

        assert_float_not_equal(v1, v2, 0.0001);

        movf(core);
        core->IP = 0;

        assert_float_equal(core->F[r1], core->F[r2], 0.0001);
        assert_float_equal(v2, core->F[r2], 0.0001);
    }

    // cas extreme des u64 ?

    // cas ou c'est pas des u64 ?
    free(file_buffer);
    free(core);
}

// v1 = v2
static void test_movv(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 15);      // Entre 0 et 14
        u8 r2 = (u8)(15 + random() % 17); // Entre 15 et 31

        for (int i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            core->V[r2][i] = (u64)rand();
        }
        
        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, 0);

        movv(core);
        core->IP = 0;

        for (int i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            assert_int_equal(core->V[r1][i], core->V[r2][i]);
        }

    }
    free(file_buffer);
    free(core);
}

// t1 = t2
static void test_movt(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 15);      // Entre 0 et 14
        u8 r2 = (u8)(15 + random() % 17); // Entre 15 et 31

        for (int i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            core->T[r2][i] = (i64)rand();
        }

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, 0);

        movt(core);
        core->IP = 0;

        for (int i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            assert_int_equal(core->T[r1][i], core->T[r2][i]);
        }
    }
    free(file_buffer);
    free(core);
}

// g1 = g2
static void test_movg(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 15);      // Entre 0 et 14
        u8 r2 = (u8)(15 + random() % 17); // Entre 15 et 31

        for (int i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            core->G[r2][i] = (f64)rand();
        }

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, 0);

        movg(core);
        core->IP = 0;

        for (int i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            assert_float_equal(core->G[r1][i], core->G[r2][i], 0.0001);
        }
    }
    free(file_buffer);
    free(core);
}

// u1 = imm1
static void test_movui(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32) + sizeof(u64);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    // U[r1] = imm (test quand U[r1] = 0 et U[r1] =! 0 si MAX_ITERATION grand)
    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u64 imm = (u64)random();

        u32 *ptr_inst = (u32 *)(core->file_buffer + core->IP);
        u64 *ptr_imm = (u64 *)(core->file_buffer + core->IP + sizeof(u32));

        *ptr_inst = create_instruction(0, 0, r1, 0, 0);
        *ptr_imm = htobe64(imm);

        movui(core);
        core->IP = 0;

        assert_int_equal(core->U[r1], imm);
    }

    free(file_buffer);
    free(core);
}

// s1 = imm1
static void test_movsi(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32) + sizeof(u64);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(rand() % 32);
        u64 imm = (u64)rand();

        u32 *ptr_inst = (u32 *)(core->file_buffer);
        i64 *ptr_imm = (i64 *)(core->file_buffer + sizeof(u32));

        *ptr_inst = create_instruction(0, 0, r1, 0, 0);
        *ptr_imm = (i64)htobe64(imm);

        movsi(core);
        core->IP = 0;

        assert_int_equal(core->S[r1], (i64)imm);
    }

    free(file_buffer);
    free(core);
}

// f1 = imm1
static void test_movfi(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32) + sizeof(u64);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    // U[r1] = imm (test quand U[r1] = 0 et U[r1] =! 0 si MAX_ITERATION grand)
    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = 2;
        f64 imm = 1.0;

        u32 *ptr_inst = (u32 *)(core->file_buffer);
        f64 *ptr_imm = (f64 *)(core->file_buffer + sizeof(u32));

        *ptr_inst = create_instruction(0, 0, r1, 0, 0);
        *ptr_imm = imm;

        movfi(core);
        core->IP = 0;

        assert_float_equal(core->F[r1], imm, 0.0001);
    }

    free(file_buffer);
    free(core);
}

// v1 = imm1, imm2 .. imm8
static void test_movvi(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32) + sizeof(u64) * NUMBER_SCALAR_IN_VECTOR_REGISTER;
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u64 imm[NUMBER_SCALAR_IN_VECTOR_REGISTER];

        for (int i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            imm[i] = (u64)rand();
        }

        u32 *ptr_inst = (u32 *)(core->file_buffer + core->IP);
        //u64 *ptr_imm = (u64 *)(core->file_buffer + core->IP + sizeof(u32));

        *ptr_inst = create_instruction(0, 0, r1, 0, 0);

        for (u64 i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            *(u64 *)(core->file_buffer + core->IP + sizeof(u32) + (i * sizeof(u64))) = imm[i];
        }

        movvi(core);
        core->IP = 0;

        for (u64 i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            assert_int_equal(htobe64(imm[i]), core->V[r1][i]);
        }
    }

    free(file_buffer);
    free(core);
}

// t1 = imm1, imm2 .. imm8
static void test_movti(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32) + sizeof(u64) * NUMBER_SCALAR_IN_VECTOR_REGISTER;
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u64 imm[NUMBER_SCALAR_IN_VECTOR_REGISTER];

        for (int i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            imm[i] = (u64)rand();
        }

        u32 *ptr_inst = (u32 *)(core->file_buffer + core->IP);
        // u64 *ptr_imm = (u64 *)(core->file_buffer + core->IP + sizeof(u32));

        *ptr_inst = create_instruction(0, 0, r1, 0, 0);

        for (u64 i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            *(i64 *)(core->file_buffer + core->IP + sizeof(u32) + (i * sizeof(i64))) = (i64)htobe64(imm[i]);
        }

        movti(core);
        core->IP = 0;

        for (u64 i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            assert_int_equal((i64)imm[i], core->T[r1][i]);
        }
    }

    free(file_buffer);
    free(core);
}

// g1 = imm1, imm2 .. imm8
static void test_movgi(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32) + sizeof(u64) * NUMBER_SCALAR_IN_VECTOR_REGISTER;
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }

    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        f64 imm[NUMBER_SCALAR_IN_VECTOR_REGISTER];

        for (int i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            imm[i] = 5.555;
        }

        u32 *ptr_inst = (u32 *)(core->file_buffer + core->IP);
        // u64 *ptr_imm = (u64 *)(core->file_buffer + core->IP + sizeof(u32));

        *ptr_inst = create_instruction(0, 0, r1, 0, 0);

        for (u64 i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            *(f64 *)(core->file_buffer + core->IP + sizeof(u32) + (i * sizeof(f64))) = imm[i];
        }

        movgi(core);
        core->IP = 0;

        for (u64 i = 0; i < NUMBER_SCALAR_IN_VECTOR_REGISTER; ++i)
        {
            assert_float_equal(imm[i], core->G[r1][i], 0.0001);
        }
    }

    free(file_buffer);
    free(core);
}

// u1 = u2 + u3
static void test_addu(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 32);
        u8 r3 = (u8)(random() % 32);

        u64 v2 = (u64)random();
        u64 v3 = (u64)random();

        core->U[r2] = v2;
        core->U[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer + core->IP);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        addu(core);
        core->IP = 0;

        if (r2 == r3)
        {
            assert_int_equal(core->U[r1], (v3 + v3));
        }
        else
        {
            assert_int_equal(core->U[r1], (v2 + v3));
        }

        // CF[0] = 1 si opération = 0
        if (core->U[r1] == 0)
        {
            assert_true(core->CF[0]);
        }
    }
    free(file_buffer);
    free(core);
}

// u1 = u2 - u3
static void test_subu(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(rand() % 10);        // entre 0 et 9
        u8 r2 = (u8)(10 + (rand() % 10)); // entre 10 et 19
        u8 r3 = (u8)(20 + (rand() % 12)); // entre 20 et 31

        u64 v2 = (u64)random();
        u64 v3 = (u64)random();

        core->U[r2] = v2;
        core->U[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer + core->IP);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        core->CF[1] = false;
        core->CF[0] = false;

        subu(core);
        core->IP = 0;

        if (v2 < v3)
        {
            assert_true(core->CF[1]);
        }

        // CF[0] = 1 si opération = 0
        if (core->U[r1] == 0)
        {
            assert_true(core->CF[0]);
        }

        if (r2 == r3)
        {
            assert_int_equal(core->U[r1], (v3 - v3));
        }
        else
        {
            assert_int_equal(core->U[r1], (v2 - v3));
        }
    }
    free(file_buffer);
    free(core);
}

// u1 = u2 & u3
static void test_andu(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 15);
        u8 r3 = (u8)(15 + random() % 17);

        u64 v2 = (u64)random();
        u64 v3 = (u64)random();

        core->U[r2] = v2;
        core->U[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        u64 expected_result = v2 & v3;

        andu(core);
        core->IP = 0;

        if (core->U[r1] == 0)
        {
            assert_true(core->CF[0]);
        }

        assert_int_equal(core->U[r1], expected_result);
    }

    free(file_buffer);
    free(core);
}

// u1 = u2 * u3
static void test_mulu(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 32);
        u8 r3 = (u8)(random() % 32);

        u64 v2 = (u64)random();
        u64 v3 = (u64)random();

        core->U[r2] = v2;
        core->U[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer + core->IP);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        mulu(core);
        core->IP = 0;

        if (r2 == r3)
        {
            assert_int_equal(core->U[r1], (v3 * v3));
        }
        else
        {
            assert_int_equal(core->U[r1], (v2 * v3));
        }

        // CF[0] = 1 si opération = 0
        if (core->U[r1] == 0)
        {
            assert_true(core->CF[0]);
        }
    }
    free(file_buffer);
    free(core);
}

// u1 = u2 / u3
static void test_divu(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    i64 rd = 1000;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 15);      // 0 à 14
        u8 r3 = (u8)(15 + random() % 17); // 15 à 31

        u64 v2 = (u64)random();
        u64 v3 = (u64)(random() % rd);

        core->U[r2] = v2;
        core->U[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        core->CF[0] = false;
        core->CF[1] = false;

        divu(core);
        core->IP = 0;

        if (v3 == 0)
        {
            // TODO : pour l'instant
            assert_int_equal(core->U[r1], 0);
            assert_true(core->CF[1]);
        }
        else if (v3 != 0)
        {
            assert_int_equal(core->U[r1], (v2 / v3));
        }

        // CF[0] = 1 si opération = 0
        if (core->U[r1] == 0)
        {
            assert_true(core->CF[0]);
        }
    }

    free(file_buffer);
    free(core);
}

// u1 = u2 % u3
static void test_modu(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    i64 rd = 1000;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 15);      // 0 à 14
        u8 r3 = (u8)(15 + random() % 17); // 15 à 31

        u64 v2 = (u64)random();
        u64 v3 = (u64)(random() % rd);

        core->U[r2] = v2;
        core->U[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        core->CF[0] = false;
        core->CF[1] = false;

        modu(core);
        core->IP = 0;

        if (v3 == 0)
        {
            // TODO : pour l'instant
            assert_int_equal(core->U[r1], 0);
            assert_true(core->CF[1]);
        }
        else if (v3 != 0)
        {
            assert_int_equal(core->U[r1], (v2 % v3));
        }

        // CF[0] = 1 si opération = 0
        if (core->U[r1] == 0)
        {
            assert_true(core->CF[0]);
        }
    }

    free(file_buffer);
    free(core);
}

// u1 += u2 * u3
static void test_fmau(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 10);
        u8 r2 = (u8)(10 + random() % 10);
        u8 r3 = (u8)(20 + random() % 12);

        u64 v1 = (u64)random();
        u64 v2 = (u64)random();
        u64 v3 = (u64)random();

        core->U[r1] = v1;
        core->U[r2] = v2;
        core->U[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer + core->IP);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        fmau(core);
        core->IP = 0;

        assert_int_equal(core->U[r1], v1 + v2 * v3);

        // CF[0] = 1 si opération = 0
        if (core->U[r1] == 0)
        {
            assert_true(core->CF[0]);
        }
    }

    free(file_buffer);
    free(core);
}

// u1 = sqrt(u2)
static void test_sqrtu(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 32);

        u64 v2 = (u64)random();
        core->U[r2] = v2;

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, 0);

        sqrtu(core);
        core->IP = 0;

        assert_int_equal(core->U[r1], (u64)sqrt((f64)v2));

        // CF[0] = 1 si opération = 0
        if (core->U[r1] == 0)
        {
            assert_true(core->CF[0]);
        }
    }

    free(file_buffer);
    free(core);
}

// u1 = log(u2)
static void test_logu(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    // manque le cas où random = 0;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 32);

        u64 v2 = 1 + ((u64)random() % (RAND_MAX - 1));
        core->U[r2] = v2;

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, 0);

        logu(core);
        core->IP = 0;

        if (v2 != 0)
        {
            assert_int_equal(core->U[r1], (u64)log10((f64)v2));
        }

        // CF[0] = 1 si opération = 0
        if (core->U[r1] == 0)
        {
            assert_true(core->CF[0]);
        }
    }

    free(file_buffer);
    free(core);
}

// u1++
static void test_incu(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u64 v1 = (u64)random();

        core->U[r1] = v1;

        u32 *ptr = (u32 *)(core->file_buffer + core->IP);
        *ptr = create_instruction(0, 0, r1, 0, 0);

        incu(core);
        core->IP = 0;

        assert_int_equal(core->U[r1], v1 + 1);
    }

    free(file_buffer);
    free(core);
}

// u1--
static void test_decu(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u64 v1 = (u64)random();

        core->U[r1] = v1;

        u32 *ptr = (u32 *)(core->file_buffer + core->IP);
        *ptr = create_instruction(0, 0, r1, 0, 0);

        decu(core);
        core->IP = 0;
        if (core->U[r1] == 0)
        {
            assert_true(core->CF[0]);
        }
        assert_int_equal(core->U[r1], v1 - 1);
    }

    free(file_buffer);
    free(core);
}

// u1 = u2 | u3
static void test_oru(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 15);
        u8 r3 = (u8)(15 + random() % 17);

        u64 v2 = (u64)random();
        u64 v3 = (u64)random();

        core->U[r2] = v2;
        core->U[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        u64 expected_result = v2 | v3;

        oru(core);
        core->IP = 0;

        if (core->U[r1] == 0)
        {
            assert_true(core->CF[0]);
        }

        assert_int_equal(core->U[r1], expected_result);
    }

    free(file_buffer);
    free(core);
}

// u1 = u2 ^ u3
static void test_xoru(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 15);
        u8 r3 = (u8)(15 + random() % 17);

        u64 v2 = (u64)random();
        u64 v3 = (u64)random();

        core->U[r2] = v2;
        core->U[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        u64 expected_result = v2 ^ v3;

        xoru(core);
        core->IP = 0;

        if (core->U[r1] == 0)
        {
            assert_true(core->CF[0]);
        }

        assert_int_equal(core->U[r1], expected_result);
    }

    free(file_buffer);
    free(core);
}

// s1 = s2 + s3
static void test_adds(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 32);
        u8 r3 = (u8)(random() % 32);

        i64 v2 = (i64)random();
        i64 v3 = (i64)random();

        core->S[r2] = v2;
        core->S[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer + core->IP);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        adds(core);
        core->IP = 0;

        if (r2 == r3)
        {
            assert_int_equal(core->S[r1], (v3 + v3));
        }
        else
        {
            assert_int_equal(core->S[r1], (v2 + v3));
        }

        // CF[0] = 1 si opération = 0
        if (core->S[r1] == 0)
        {
            assert_true(core->CF[0]);
        }
    }
    free(file_buffer);
    free(core);
}

// s1 = s2 - s3
static void test_subs(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(rand() % 10);        // entre 0 et 9
        u8 r2 = (u8)(10 + (rand() % 10)); // entre 10 et 19
        u8 r3 = (u8)(20 + (rand() % 12)); // entre 20 et 31

        i64 v2 = (i64)random();
        i64 v3 = (i64)random();

        core->S[r2] = v2;
        core->S[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer + core->IP);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        core->CF[1] = false;
        core->CF[0] = false;

        subs(core);
        core->IP = 0;

        if (v2 < v3)
        {
            assert_true(core->CF[1]);
        }

        // CF[0] = 1 si opération = 0
        if (core->S[r1] == 0)
        {
            assert_true(core->CF[0]);
        }

        if (r2 == r3)
        {
            assert_int_equal(core->S[r1], (v3 - v3));
        }
        else
        {
            assert_int_equal(core->S[r1], (v2 - v3));
        }
    }
    free(file_buffer);
    free(core);
}

// s1 = s2 * s3
static void test_muls(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 32);
        u8 r3 = (u8)(random() % 32);

        i64 v2 = (i64)random();
        i64 v3 = (i64)random();

        core->S[r2] = v2;
        core->S[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer + core->IP);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        muls(core);
        core->IP = 0;

        if (r2 == r3)
        {
            assert_int_equal(core->S[r1], (v3 * v3));
        }
        else
        {
            assert_int_equal(core->S[r1], (v2 * v3));
        }

        // CF[0] = 1 si opération = 0
        if (core->S[r1] == 0)
        {
            assert_true(core->CF[0]);
        }
    }
    free(file_buffer);
    free(core);
}

// s1 = s2 / s3
static void test_divs(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    i64 rd = 1000;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 15);      // 0 à 14
        u8 r3 = (u8)(15 + random() % 17); // 15 à 31

        i64 v2 = (i64)random();
        i64 v3 = (i64)(random() % rd);

        core->S[r2] = v2;
        core->S[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        core->CF[0] = false;
        core->CF[1] = false;

        divs(core);
        core->IP = 0;

        if (v3 == 0)
        {
            // TODO : pour l'instant
            assert_int_equal(core->S[r1], 0);
            assert_true(core->CF[1]);
        }
        else if (v3 != 0)
        {
            assert_int_equal(core->S[r1], (v2 / v3));
        }

        // CF[0] = 1 si opération = 0
        if (core->S[r1] == 0)
        {
            assert_true(core->CF[0]);
        }
    }

    free(file_buffer);
    free(core);
}

// s1 = s2 % s3
static void test_mods(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    i64 rd = 1000;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 15);      // 0 à 14
        u8 r3 = (u8)(15 + random() % 17); // 15 à 31

        i64 v2 = (i64)random();
        i64 v3 = (i64)(random() % rd);

        core->S[r2] = v2;
        core->S[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        core->CF[0] = false;
        core->CF[1] = false;

        mods(core);
        core->IP = 0;

        if (v3 == 0)
        {
            // TODO : pour l'instant
            assert_int_equal(core->S[r1], 0);
            assert_true(core->CF[1]);
        }
        else if (v3 != 0)
        {
            assert_int_equal(core->S[r1], (v2 % v3));
        }

        // CF[0] = 1 si opération = 0
        if (core->S[r1] == 0)
        {
            assert_true(core->CF[0]);
        }
    }

    free(file_buffer);
    free(core);
}

// s1 += s2 * s3
static void test_fmas(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 10);
        u8 r2 = (u8)(10 + random() % 10);
        u8 r3 = (u8)(20 + random() % 12);

        i64 v1 = (i64)random();
        i64 v2 = (i64)random();
        i64 v3 = (i64)random();

        core->S[r1] = v1;
        core->S[r2] = v2;
        core->S[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer + core->IP);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        fmas(core);
        core->IP = 0;

        assert_int_equal(core->S[r1], (v1 + (v2 * v3)));

        // CF[0] = 1 si opération = 0
        if (core->S[r1] == 0)
        {
            assert_true(core->CF[0]);
        }
    }

    free(file_buffer);
    free(core);
}

// s1 = sqrt(s2)
static void test_sqrts(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 32);

        i64 v2 = (i64)random();
        core->S[r2] = v2;

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, 0);

        sqrts(core);
        core->IP = 0;

        assert_int_equal(core->S[r1], (i64)sqrt((f64)v2));

        // CF[0] = 1 si opération = 0
        if (core->S[r1] == 0)
        {
            assert_true(core->CF[0]);
        }
    }

    free(file_buffer);
    free(core);
}

// s1 = log(s2)
static void test_logs(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    // manque le cas où random = 0;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 32);

        i64 v2 = 1 + ((i64)random() % (RAND_MAX - 1));
        core->S[r2] = v2;

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, 0);

        logs(core);
        core->IP = 0;

        if (v2 != 0)
        {
            assert_int_equal(core->S[r1], (i64)log10((f64)v2));
        }

        // CF[0] = 1 si opération = 0
        if (core->S[r1] == 0)
        {
            assert_true(core->CF[0]);
        }
    }

    free(file_buffer);
    free(core);
}

// s1 = s2 & s3
static void test_ands(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 15);
        u8 r3 = (u8)(15 + random() % 17);

        i64 v2 = (i64)random();
        i64 v3 = (i64)random();

        core->S[r2] = v2;
        core->S[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        i64 expected_result = v2 & v3;

        ands(core);
        core->IP = 0;

        if (core->S[r1] == 0)
        {
            assert_true(core->CF[0]);
        }

        assert_int_equal(core->S[r1], expected_result);
    }

    free(file_buffer);
    free(core);
}

// s1 = s2 | s3
static void test_ors(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 15);
        u8 r3 = (u8)(15 + random() % 17);

        i64 v2 = (i64)random();
        i64 v3 = (i64)random();

        core->S[r2] = v2;
        core->S[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        i64 expected_result = v2 | v3;

        ors(core);
        core->IP = 0;

        if (core->S[r1] == 0)
        {
            assert_true(core->CF[0]);
        }

        assert_int_equal(core->S[r1], expected_result);
    }

    free(file_buffer);
    free(core);
}

// s1 = s2 ^ s3
static void test_xors(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 15);
        u8 r3 = (u8)(15 + random() % 17);

        i64 v2 = (i64)random();
        i64 v3 = (i64)random();

        core->S[r2] = v2;
        core->S[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        i64 expected_result = v2 ^ v3;

        xors(core);
        core->IP = 0;

        if (core->S[r1] == 0)
        {
            assert_true(core->CF[0]);
        }

        assert_int_equal(core->S[r1], expected_result);
    }

    free(file_buffer);
    free(core);
}

// f1 = f2 + f3
static void test_addf(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 32);
        u8 r3 = (u8)(random() % 32);

        f64 v2 = (f64)random();
        f64 v3 = (f64)random();

        core->F[r2] = v2;
        core->F[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer + core->IP);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        addf(core);
        core->IP = 0;

        if (r2 == r3)
        {
            assert_float_equal(core->F[r1], (v3 + v3), 0.0001);
        }
        else
        {
            assert_float_equal(core->F[r1], (v2 + v3), 0.0001);
        }

        // CF[0] = 1 si opération = 0
        if (core->F[r1] == 0)
        {
            assert_true(core->CF[0]);
        }
    }
    free(file_buffer);
    free(core);
}

// f1 = f2 - f3
static void test_subf(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(rand() % 10);        // entre 0 et 9
        u8 r2 = (u8)(10 + (rand() % 10)); // entre 10 et 19
        u8 r3 = (u8)(20 + (rand() % 12)); // entre 20 et 31

        f64 v2 = (f64)random();
        f64 v3 = (f64)random();

        core->F[r2] = v2;
        core->F[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer + core->IP);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        core->CF[1] = false;
        core->CF[0] = false;

        subf(core);
        core->IP = 0;

        if (v2 < v3)
        {
            assert_true(core->CF[1]);
        }

        // CF[0] = 1 si opération = 0
        if (core->F[r1] == 0)
        {
            assert_true(core->CF[0]);
        }

        if (r2 == r3)
        {
            assert_float_equal(core->F[r1], (v3 - v3), 0.0001);
        }
        else
        {
            assert_float_equal(core->F[r1], (v2 - v3), 0.0001);
        }
    }
    free(file_buffer);
    free(core);
}

// f1 = f2 * f3
static void test_mulf(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 32);
        u8 r3 = (u8)(random() % 32);

        f64 v2 = (f64)random();
        f64 v3 = (f64)random();

        core->F[r2] = v2;
        core->F[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer + core->IP);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        mulf(core);
        core->IP = 0;

        if (r2 == r3)
        {
            assert_float_equal(core->F[r1], (v3 * v3), 0.0001);
        }
        else
        {
            assert_float_equal(core->F[r1], (v2 * v3), 0.0001);
        }

        // CF[0] = 1 si opération = 0
        if (core->F[r1] == 0)
        {
            assert_true(core->CF[0]);
        }
    }
    free(file_buffer);
    free(core);
}

// f1 = f2 / f3
static void test_divf(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    i64 rd = 1000;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 15);      // 0 à 14
        u8 r3 = (u8)(15 + random() % 17); // 15 à 31

        f64 v2 = (f64)random();
        f64 v3 = (f64)(random() % rd);

        core->F[r2] = v2;
        core->F[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        core->CF[0] = false;
        core->CF[1] = false;

        divf(core);
        core->IP = 0;

        if (v3 == 0)
        {
            // TODO : pour l'instant
            assert_float_equal(core->F[r1], 0, 0.0001);
            assert_true(core->CF[1]);
        }
        else if (v3 != 0)
        {
            assert_float_equal(core->F[r1], (v2 / v3), 0.0001);
        }

        // CF[0] = 1 si opération = 0
        if (core->F[r1] == 0)
        {
            assert_true(core->CF[0]);
        }
    }

    free(file_buffer);
    free(core);
}

// f1 += f2 * f3
static void test_fmaf_(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 10);
        u8 r2 = (u8)(10 + random() % 10);
        u8 r3 = (u8)(20 + random() % 12);

        f64 v1 = (f64)random();
        f64 v2 = (f64)random();
        f64 v3 = (f64)random();

        core->F[r1] = v1;
        core->F[r2] = v2;
        core->F[r3] = v3;

        u32 *ptr = (u32 *)(core->file_buffer + core->IP);
        *ptr = create_instruction(0, 0, r1, r2, r3);

        fmaf_(core);
        core->IP = 0;

        assert_float_equal(core->F[r1], (v1 + (v2 * v3)), 0.00001);

        // CF[0] = 1 si opération = 0
        if (core->F[r1] == 0)
        {
            assert_true(core->CF[0]);
        }
    }

    free(file_buffer);
    free(core);
}

// f1 = sqrt(f2)
static void test_sqrtf_(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 32);

        f64 v2 = (f64)random();
        core->F[r2] = v2;

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, 0);

        sqrtf_(core);
        core->IP = 0;

        assert_float_equal(core->F[r1], sqrt(v2), 0.0001);

        // CF[0] = 1 si opération = 0
        if (core->F[r1] == 0)
        {
            assert_true(core->CF[0]);
        }
    }

    free(file_buffer);
    free(core);
}

// f1 = log(f2)
static void test_logf_(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    // manque le cas où random = 0;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);
        u8 r2 = (u8)(random() % 32);

        f64 v2 = (f64)(1 + (random() % (RAND_MAX - 1))) / 2.453541;
        core->F[r2] = v2;

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, 0);

        logf_(core);
        core->IP = 0;

        if (v2 != 0)
        {
            assert_float_equal(core->F[r1], log10(v2), 0.0001);
        }

        // CF[0] = 1 si opération = 0
        if (core->F[r1] == 0)
        {
            assert_true(core->CF[0]);
        }
    }

    free(file_buffer);
    free(core);
}

// comparaison unisgned int
static void test_cmpu(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    // U[r1] == U[r2]
    for (int i = 0; i < MAX_ITERATION; ++i)
    {

        u8 r1 = (u8)(rand() % 32);
        u8 r2 = (u8)(rand() % 32);

        u64 v1 = (u64)rand();

        core->U[r1] = v1;
        core->U[r2] = v1;

        u32 *ptr = (u32 *)(core->file_buffer + core->IP);
        *ptr = create_instruction(0, 0, r1, r2, 0);

        cmpu(core);
        core->IP = 0;

        assert_true(core->CF[7]);
        assert_false(core->CF[6]);
        assert_false(core->CF[5]);
        assert_true(core->CF[4]);
        assert_true(core->CF[2]);
        assert_false(core->CF[1]);
        assert_false(core->CF[0]);
    }

    // U[r1] > U[r2]
    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(rand() % 15);
        u8 r2 = (u8)(15 + (rand() % 17));

        u64 v1 = (u64)((rand() % (RAND_MAX / 2)) + (RAND_MAX / 2));
        u64 v2 = (u64)(rand() % (RAND_MAX / 2));

        core->U[r1] = v1;
        core->U[r2] = v2;

        u32 *ptr = (u32 *)(core->file_buffer);
        *ptr = create_instruction(0, 0, r1, r2, 0);

        cmpu(core);
        core->IP = 0;

        assert_false(core->CF[7]);
        assert_true(core->CF[6]);
        assert_true(core->CF[5]);
        assert_true(core->CF[4]);
        assert_false(core->CF[2]);
        assert_false(core->CF[1]);
        assert_false(core->CF[0]);
    }

    free(file_buffer);
    free(core);
}

// jump if equal
static void test_je(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t rd = 100;
    size_t file_size = (sizeof(u32) + sizeof(u64)) * rd;
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    // dépassement ?
    // A GERER ==> DONNE PROBLEME MALLOC DEVRAIT GERER LE FAIT DE DEPASSER, MESSAGE DERREUR??

    // Test sans dépassement
    // Doit jumper
    core->CF[7] = true;
    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        core->IP = 0;

        // TODO : vérifier que ça dépasse vraiment pas la taille du buffer
        u8 size = (u8)(rand() % (int)file_size);

        u64 *ptr_addr = (u64 *)(core->file_buffer + core->IP + sizeof(u32));
        *ptr_addr = htobe64(size);

        je(core);

        assert_int_equal(core->IP, size);
    }

    // Ne doit pas jumper
    core->CF[7] = false;
    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        core->IP = 0;
        // TODO : vérifier que ça dépasse vraiment pas la taille du buffer
        u8 size = (u8)(rand() % (int)file_size);
        u64 *ptr_addr = (u64 *)(core->file_buffer + core->IP + sizeof(u32));
        *ptr_addr = htobe64(size);

        je(core);

        assert_int_equal(core->IP, 12);
    }

    free(file_buffer);
    free(core);
}

// jump if not equal
static void test_jne(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t rd = 100;
    size_t file_size = (sizeof(u32) + sizeof(u64)) * rd;
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    // dépassement ?
    // A GERER ==> DONNE PROBLEME MALLOC DEVRAIT GERER LE FAIT DE DEPASSER, MESSAGE DERREUR??

    // Test sans dépassement

    // Doit jumper
    core->CF[6] = true;

    // doit jumper

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        core->IP = 0;

        // TODO : vérifier que ça dépasse vraiment pas la taille du buffer
        u8 size = (u8)(rand() % (int)file_size);

        u64 *ptr_addr = (u64 *)(core->file_buffer + core->IP + sizeof(u32));
        *ptr_addr = htobe64(size);

        jne(core);

        assert_int_equal(core->IP, size);
    }

    // Ne doit pas jumper
    core->CF[6] = false;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        core->IP = 0;
        // TODO : vérifier que ça dépasse vraiment pas la taille du buffer
        u8 size = (u8)(rand() % (int)file_size);
        u64 *ptr_addr = (u64 *)(core->file_buffer + core->IP + sizeof(u32));
        *ptr_addr = htobe64(size);

        jne(core);

        assert_int_equal(core->IP, 12);
    }

    free(file_buffer);
    free(core);
}

// jump
static void test_jge(void **state)
{

    (void)state;
    core_t *core = core_init();

    size_t rd = 100;
    size_t file_size = (sizeof(u32) + sizeof(u64)) * rd;
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    // dépassement ?
    // A GERER ==> DONNE PROBLEME MALLOC DEVRAIT GERER LE FAIT DE DEPASSER, MESSAGE DERREUR??

    // Test sans dépassement

    // Doit jumper
    core->CF[4] = true;

    // doit jumper

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        core->IP = 0;

        // TODO : vérifier que ça dépasse vraiment pas la taille du buffer
        u8 size = (u8)(rand() % (int)file_size);

        u64 *ptr_addr = (u64 *)(core->file_buffer + core->IP + sizeof(u32));
        *ptr_addr = htobe64(size);

        jge(core);

        assert_int_equal(core->IP, size);
    }

    // Ne doit pas jumper
    core->CF[4] = false;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        core->IP = 0;
        // TODO : vérifier que ça dépasse vraiment pas la taille du buffer
        u8 size = (u8)(rand() % (int)file_size);
        u64 *ptr_addr = (u64 *)(core->file_buffer + core->IP + sizeof(u32));
        *ptr_addr = htobe64(size);

        jge(core);

        assert_int_equal(core->IP, 12);
    }

    free(file_buffer);
    free(core);
}

// jump if lower
static void test_jl(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t rd = 100;
    size_t file_size = (sizeof(u32) + sizeof(u64)) * rd;
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    // dépassement ?
    // A GERER ==> DONNE PROBLEME MALLOC DEVRAIT GERER LE FAIT DE DEPASSER, MESSAGE DERREUR??

    // Test sans dépassement
    // Doit jumper
    core->CF[3] = true;
    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        core->IP = 0;

        // TODO : vérifier que ça dépasse vraiment pas la taille du buffer
        u8 size = (u8)(rand() % (int)file_size);

        u64 *ptr_addr = (u64 *)(core->file_buffer + core->IP + sizeof(u32));
        *ptr_addr = htobe64(size);

        jl(core);

        assert_int_equal(core->IP, size);
    }

    // Ne doit pas jumper
    core->CF[3] = false;
    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        core->IP = 0;
        // TODO : vérifier que ça dépasse vraiment pas la taille du buffer
        u8 size = (u8)(rand() % (int)file_size);
        u64 *ptr_addr = (u64 *)(core->file_buffer + core->IP + sizeof(u32));
        *ptr_addr = htobe64(size);

        jl(core);

        assert_int_equal(core->IP, 12);
    }

    free(file_buffer);
    free(core);
}

// 
static void test_jle(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t rd = 100;
    size_t file_size = (sizeof(u32) + sizeof(u64)) * rd;
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    // dépassement ?
    // A GERER ==> DONNE PROBLEME MALLOC DEVRAIT GERER LE FAIT DE DEPASSER, MESSAGE DERREUR??

    // Test sans dépassement
    // Doit jumper
    core->CF[2] = true;
    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        core->IP = 0;

        // TODO : vérifier que ça dépasse vraiment pas la taille du buffer
        u8 size = (u8)(rand() % (int)file_size);

        u64 *ptr_addr = (u64 *)(core->file_buffer + core->IP + sizeof(u32));
        *ptr_addr = htobe64(size);

        jle(core);

        assert_int_equal(core->IP, size);
    }

    // Ne doit pas jumper
    core->CF[2] = false;
    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        core->IP = 0;
        // TODO : vérifier que ça dépasse vraiment pas la taille du buffer
        u8 size = (u8)(rand() % (int)file_size);
        u64 *ptr_addr = (u64 *)(core->file_buffer + core->IP + sizeof(u32));
        *ptr_addr = htobe64(size);

        jle(core);

        assert_int_equal(core->IP, 12);
    }

    free(file_buffer);
    free(core);
}

// 
static void test_jz(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t rd = 100;
    size_t file_size = (sizeof(u32) + sizeof(u64)) * rd;
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    // dépassement ?
    // A GERER ==> DONNE PROBLEME MALLOC DEVRAIT GERER LE FAIT DE DEPASSER, MESSAGE DERREUR??

    // Test sans dépassement
    // Doit jumper
    core->CF[0] = true;
    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        core->IP = 0;

        // TODO : vérifier que ça dépasse vraiment pas la taille du buffer
        u8 size = (u8)(rand() % (int)file_size);

        u64 *ptr_addr = (u64 *)(core->file_buffer + core->IP + sizeof(u32));
        *ptr_addr = htobe64(size);

        jz(core);

        assert_int_equal(core->IP, size);
    }

    // Ne doit pas jumper
    core->CF[0] = false;
    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        core->IP = 0;
        // TODO : vérifier que ça dépasse vraiment pas la taille du buffer
        u8 size = (u8)(rand() % (int)file_size);
        u64 *ptr_addr = (u64 *)(core->file_buffer + core->IP + sizeof(u32));
        *ptr_addr = htobe64(size);

        jz(core);

        assert_int_equal(core->IP, 12);
    }

    free(file_buffer);
    free(core);
}

// 
static void test_jnz(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t rd = 100;
    size_t file_size = (sizeof(u32) + sizeof(u64)) * rd;
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    // dépassement ?
    // A GERER ==> DONNE PROBLEME MALLOC DEVRAIT GERER LE FAIT DE DEPASSER, MESSAGE DERREUR??

    // Test sans dépassement
    // Doit jumper
    core->CF[0] = false;
    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        core->IP = 0;

        // TODO : vérifier que ça dépasse vraiment pas la taille du buffer
        u8 size = (u8)(rand() % (int)file_size);

        u64 *ptr_addr = (u64 *)(core->file_buffer + core->IP + sizeof(u32));
        *ptr_addr = htobe64(size);

        jnz(core);

        assert_int_equal(core->IP, size);
    }

    // Ne doit pas jumper
    core->CF[0] = true;
    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        core->IP = 0;
        // TODO : vérifier que ça dépasse vraiment pas la taille du buffer
        u8 size = (u8)(rand() % (int)file_size);
        u64 *ptr_addr = (u64 *)(core->file_buffer + core->IP + sizeof(u32));
        *ptr_addr = htobe64(size);

        jnz(core);

        assert_int_equal(core->IP, 12);
    }

    free(file_buffer);
    free(core);
}

//
/*static void test_outu(void **state)
{
    core_t* core = core_init();
    u8 * chaine = "hello";
    core->U[0] = (u64)(*chaine);

    u64 *ptr = (u64 *)(core->file_buffer + core->IP);
    *ptr = 0;

    int out = open("./out.dat", O_CREAT | O_WRONLY | O_TRUNC, 0600); // peut creer des problemes
    dup2(out, STDOUT_FILENO); // aussi
    outu(core);
    dup2(STDOUT_FILENO, out);

    close(out);

    core_drop(core);
} */

//
static void test_sumg(void **state)
{
    (void)state;
    core_t *core = core_init();

    size_t file_size = sizeof(u32);
    char *file_buffer = (char *)malloc(file_size);

    if (file_buffer == NULL)
    {
        printf("failed to allocate file buffer of size %lu bytes", file_size);
        free(core);
        exit(1);
    }
    core->file_buffer = file_buffer;

    for (int i = 0; i < MAX_ITERATION; ++i)
    {
        u8 r1 = (u8)(random() % 32);

        for (int i = 0; i < 8; ++i)
        {
            core->G[r1][i] = (f64)random();
        }

        u32 *ptr = (u32 *)(core->file_buffer + core->IP);
        *ptr = create_instruction(0, 0, r1, 0, 0);

        f64 sum = 0;
        for (int i = 0; i < 8; ++i)
        {
            sum += core->G[r1][i];
        }
        sumg(core);
        core->IP = 0;
        assert_float_equal(core->G[r1][0], sum, 0.0001);
    
    }

    free(file_buffer);
    free(core);
}

//
int main(void)
{
    int result = 0;
    const struct CMUnitTest tests[] = {
        cmocka_unit_test(test_loadu),
        cmocka_unit_test(test_loads),
        cmocka_unit_test(test_loadf),
        cmocka_unit_test(test_loadv),
        cmocka_unit_test(test_loadt),
        cmocka_unit_test(test_loadg),
        cmocka_unit_test(test_storeu),
        cmocka_unit_test(test_stores),
        cmocka_unit_test(test_storef),
        cmocka_unit_test(test_storeg),
        cmocka_unit_test(test_storev),
        cmocka_unit_test(test_storet),
        cmocka_unit_test(test_movu),
        cmocka_unit_test(test_movs),
        cmocka_unit_test(test_movf),
        cmocka_unit_test(test_movv),
        cmocka_unit_test(test_movt),
        cmocka_unit_test(test_movg),
        cmocka_unit_test(test_movui),
        cmocka_unit_test(test_movsi),
        cmocka_unit_test(test_movfi),
        cmocka_unit_test(test_movvi),
        cmocka_unit_test(test_movti),
        cmocka_unit_test(test_movgi),
        cmocka_unit_test(test_addu),
        cmocka_unit_test(test_subu),
        cmocka_unit_test(test_mulu),
        cmocka_unit_test(test_divu),
        cmocka_unit_test(test_modu),
        cmocka_unit_test(test_fmau),
        cmocka_unit_test(test_sqrtu),
        cmocka_unit_test(test_logu),
        cmocka_unit_test(test_incu),
        cmocka_unit_test(test_decu),
        cmocka_unit_test(test_andu),
        cmocka_unit_test(test_oru),
        cmocka_unit_test(test_xoru),
        cmocka_unit_test(test_adds),
        cmocka_unit_test(test_subs),
        cmocka_unit_test(test_muls),
        cmocka_unit_test(test_divs),
        cmocka_unit_test(test_mods),
        cmocka_unit_test(test_fmas),
        cmocka_unit_test(test_sqrts),
        cmocka_unit_test(test_logs),
        cmocka_unit_test(test_ands),
        cmocka_unit_test(test_ors),
        cmocka_unit_test(test_xors),
        cmocka_unit_test(test_addf),
        cmocka_unit_test(test_subf),
        cmocka_unit_test(test_mulf),
        cmocka_unit_test(test_divf),
        cmocka_unit_test(test_fmaf_),
        cmocka_unit_test(test_sqrtf_),
        cmocka_unit_test(test_logf_),
        cmocka_unit_test(test_cmpu),
        cmocka_unit_test(test_je),
        cmocka_unit_test(test_jne),
        cmocka_unit_test(test_jge),
        cmocka_unit_test(test_jl),
        cmocka_unit_test(test_jle),
        cmocka_unit_test(test_jz),
        cmocka_unit_test(test_jnz),
        cmocka_unit_test(test_sumg),
        // cmocka_unit_test(test_outu),
    };
    result |= cmocka_run_group_tests_name("vm", tests, NULL, NULL);

    return result;
}
